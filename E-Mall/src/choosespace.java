...
/**
 * Servlet implementation class choosespace
 */
@WebServlet("/choosespace")
public class choosespace extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session != null) {
			...
			out.println("<!DOCTYPE html>\r\n" + 
					...
					"<title>E-MALL_MARKETING-STAFF-BOOK-AVAILABLE-BUSINESS-SPACE</title>\r\n" + 
					...
					"    <a href=\"viewspaceowner\" class=\"w3-bar-item w3-button w3-padding w3-blue\">&nbsp; Book Available Business Space</a>\r\n" + 
					...
					"    <h5><b> Book Available Business Space</b></h5>\r\n" + 
					"  </header>\r\n" + 
					"\r\n" + 
					"  <div class=\"w3-row-padding w3-margin-bottom\">");
			out.println("<form method=\"post\" action=\"bookspace\">\r\n" + "			<table>\r\n"
					+ "				<tr>\r\n" + "					<td>SPACE ID</td>\r\n"
					+ "					<td><input type=\"text\" name=\"id\"\r\n"
					+ "						placeholder=\"Enter Space ID\" required></td>\r\n"
					+ "				</tr>\r\n" +

					"<tr><td> UNIT(S)</td><td><input type=\"text\" name=\"unit\" placeholder=\"Enter Unit(s) Number\" required></td></tr><tr><td>DURATION</td>"
					+ "<td><input type=\"date\" name=\"startdate\" placeholder=\"Start Date\" required>&nbspTO&nbsp<input type=\"date\" name=\"enddate\" placeholder=\"End Date\" required></td></tr>"
					+
			...
			try {
				...
				ResultSet rs = stmt.executeQuery("select * from choosespace1");
				out.println("<table border=1 width=50% height=50%>");
				out.println(
						"<tr><th>Id</th><th>Space</th><th>Unit(s)</th><th>Unit(s) Occupied</th><tr>");
				while (rs.next()) {
					int id = rs.getInt("id");
					...
					int unitoccupied = rs.getInt("SUM(s.units)");

					out.println("<tr><td>" + id + "</td><td>" + space + "</td><td>"  + unit + "</td><td>"  + unitoccupied + "</td></tr>");
				}
